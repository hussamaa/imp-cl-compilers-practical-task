#!/bin/bash
# 
# Evalute results after kernel generation (detailed)
#
...
# History:
#
#  v0.1 2017-11-08, Hussama Ismail: 
#     - Initial Version
#
...
for kernel in $KERNEL_LIST; do
  ...
  if [ "$sha_kernel_optimized" != "$sha_kernel_non_optimized" ]; then
    echo ""
    echo -e "* log from: \033[33;33m$kernel_optimized_log_file\e[0m"
    cat $kernel_optimized_log_file 
    echo ""
    echo -e "* log from: \033[33;33m$kernel_non_optimized_log_file\e[0m"
    cat $kernel_non_optimized_log_file
    sleep 1
  fi
